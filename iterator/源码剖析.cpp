...
//注解:这是五种迭代器的类别,用来告诉算法,迭代器的类型,和可以使用的操作,相当于一种表示符,之所以设计成类型,是为了与 iterator_traist
//结合使用,iterator_traits 见书签1 ,关于类型的作用见文件iterator_tag.cpp 
...
/*
*从49-100 是针对每一种类型的迭代器,都需要进行定义的5中基本的类型成员,以便配合萃取器进行使用,关于为什么要使用萃取器
*而不是直接typenaem itr::iterator_category,再traist处介绍 
*需要定于的五种类型:
*1.迭代器的category
*2.迭代器所指向的元素的类型
*3.迭代器相减的类型
...
/*
关于iterator_taraits设计的理念,对使用者透明,我们知道,用户传给算法的可能是一个Iterator也可能是一个退化的指针,如果我们直接问,那么
指针是无法自己回答出自己的类型的,所以需要一个第三方介质协助回答,对于一个泛化的迭代器,我们让迭代器自己回答,对于一个指针,我们用,萃取器帮他
回答.
实现的原理:
模板的泛化与偏特化.
迭代器使用泛化的,指针使用偏特化的 
...
#ifdef __STL_CLASS_PARTIAL_SPECIALIZATION
//返回两个迭代器之间的距离 
//对于非随机迭代器,需要再实现自身迭代器是,对++进行重载,定义自己的迭代器规则 
...
#endif
//因为双向指针,可以给正的表示前进,可以给负的,表示向后,所以要单独设计 
...
// std::back_insert_iterator 后附到为之创建它的容器的输出迭代器 (LegacyOutputIterator) 
//凡在赋值迭代器时调用容器的 push_back() 成员函数（无论是否解引用）。自增 std::back_insert_iterator 是无操作。
//用法见back_insert_iterator.cpp 
...
// 插入迭代器,调用容器内部的insert()函数,对于++,--不做任何操作而是在operator =()中调用Insert 
template <class _Container>
